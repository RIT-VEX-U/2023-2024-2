...
bool CataSys::intake_running() { return !(intake_sys.current_state() == IntakeState::Stopped); }

bool CataSys::still_dropping() {
  bool still_dropping = cata_sys.current_state() == CataOnlyState::WaitingForDrop ||
...
bool CataSys::can_fire() const { return cata_sys.current_state() == CataOnlyState::ReadyToFire; }

class CataSysPage : public screen::Page {
public:
  CataSysPage(const CataSys &cs) : gd(30, 130.0, 270.0, {vex::green, vex::red}, 2), cs(cs) {}
  ...
  void draw(vex::brain::lcd &scr, bool, unsigned int) override {
    ...
    gd.add_samples({cs.cata_pot.angle(vex::deg), cs.cata_sys.pid.get_target()});
    const bool ball_in_intake = cs.intake_watcher.objectDistance(distanceUnits::mm) < intake_sensor_dist_mm;
    ...
    scr.printAt(40, 120, true, "Cata Temp: %.0fC", cs.cata_motor.temperature(vex::temperatureUnits::celsius));
...
AutoCommand *CataSys::WaitForHold() {
  return new FunctionCommand([&]() { return intake_sys.current_state() == IntakeState::Stopped; });
}
...
CataOnlyState CataSys::get_cata_state() { return cata_sys.current_state(); }

IntakeState CataSys::get_intake_state() { return intake_sys.current_state(); }