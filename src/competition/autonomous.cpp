...
void scoreAutoFull() {
  ...
  CommandController cmd{
      ...
      cata_sys.IntakeToHold(),
      drive_sys
          .PurePursuitCmd(drive_mc_slow,
                          PurePursuit::Path({{.x = 124, .y = 43}, {.x = 124, .y = 35}, {.x = 129, .y = 26}}, 4), FWD)
          ->withTimeout(1),
      ...
      drive_sys.PurePursuitCmd(drive_mc_slow,
                               PurePursuit::Path(
                                   {
                                       {.x = 127, .y = 30},
                                       {.x = 109, .y = 30},
      ...
      new Branch{new VisionObjectExists(vision_filter_s{
                     .min_area = 2000,
                     .max_area = 100000,
                     .aspect_low = 0.5,
                     .aspect_high = 2,
                 ...
                 new InOrder{
                     // Object doesn't exist, big sad (turn & continue on)
                     new FunctionCommand(light_on),
                     drive_sys.TurnToHeadingCmd(123),
                 },
                 new InOrder{
                     // Object exists, go get 'em!
                     // Track
                     cata_sys.IntakeToHold(),
      ...
      new Branch{new FunctionCondition([]() { return intake_watcher.objectDistance(mm) < 150; }),
                 new InOrder{
                     // No ball detected
                     // do later, shouldn't get here
                 },
                 new InOrder{
                     // Ball detected, score it
                     drive_sys.TurnToHeadingCmd(17, 0.4),
                     new Async{new InOrder{new DelayCommand(200), cata_sys.Unintake()}},
  ...
  CommandController cmd{
      ...
      (new RepeatUntil(
           InOrder{
               ...
               drive_sys.PurePursuitCmd(drive_pid,
                                        PurePursuit::Path(
                                            {
                                                {.x = 15, .y = 121},
                                                {.x = 17, .y = 120},
